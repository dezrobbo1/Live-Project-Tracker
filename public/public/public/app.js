// ======= CSV Import: tolerant headers + delimiter detection =======
...
// ======= State & Storage =======
const state = { allTasks: [], viewTasks: [], deptFilter: 'All' };
const LS_KEY='live-tracker-state';
function saveLocal(){ localStorage.setItem(LS_KEY, JSON.stringify({ allTasks: state.allTasks, deptFilter: state.deptFilter })); }
function loadLocal(){ try{ const s=JSON.parse(localStorage.getItem(LS_KEY)||'{}'); if(Array.isArray(s.allTasks)) state.allTasks=s.allTasks; if(s.deptFilter) state.deptFilter=s.deptFilter; }catch{} }
...
deptFilter.addEventListener('change', ()=>{
  state.deptFilter=deptFilter.value; saveLocal(); render();
});
importBtn.addEventListener('click', ()=>fileInput.click());
fileInput.addEventListener('change', async (e)=>{
  const file=e.target.files[0]; if(!file) return;
  await handleCsvFile(file); e.target.value='';
});
exportBtn.addEventListener('click', exportCsv);
clearBtn.addEventListener('click', ()=>{
  if(confirm('Clear current project data?')){ state.allTasks=[]; saveLocal(); render(); }
});
...
function render(){
  ...
  const filtered = state.allTasks.filter(t=>{
    ...
    const pf=new Date(t.PlannedFinish).getTime();
    return overlaps(ps,pf,{start,end});
  }).sort((a,b)=> new Date(a.PlannedStart)-new Date(b.PlannedStart));

  const rows = state.deptFilter==='All'
    ? filtered
    : filtered.filter(t => (t.Department||'').toLowerCase().includes(state.deptFilter.toLowerCase()));
...
window.startTask=startTask; window.pauseTask=pauseTask; window.finishTask=finishTask;
...
async function handleCsvFile(file){
  ...
  state.allTasks=out; saveLocal(); render();
}
...
// Init
loadLocal();
document.getElementById('deptFilter').value = state.deptFilter || 'All';
render();
